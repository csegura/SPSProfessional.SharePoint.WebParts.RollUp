...
<xsl:stylesheet version="1.0" 
        ...
        <tr class="" >
          <td class="ms-vb-title">
            <xsl:call-template name="ContextMenu">
              <xsl:with-param name="content" select="_RowNumber" />
            </xsl:call-template>         
          </td>
  ...
  <xsl:template name="ContextMenuScript">
    ...
    <xsl:value-of select="sps:MenuScriptBegin()" />
    <![CDATA[
        var selectRow = 
        "]]><xsl:value-of select="sps:EventJS('Select','currentItemID')" /><![CDATA[";
        // Put CAMOpt menus here
        CAMOpt(m, "Select Row", selectRow, "");
        CAMSep(m);
    ]]>
    <xsl:value-of select="sps:MenuScriptEnd()" />
    <![CDATA[
    // Put Custom javascript functions below
